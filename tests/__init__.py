...
def compare_req_resp(req_obj, resp_obj):
    for k, v in req_obj.items():
        assert k in resp_obj.keys(), "Key {!r} not in response (keys are {!r})".format(
            k, resp_obj.keys()
        )
        assert resp_obj[k] == v, "Value for key {!r} should be {!r} but is {!r}".format(
            k, v, resp_obj[k]
        )
...
class MongoRestTestCase(unittest.TestCase):
    ...
    def test_model_validation_unicode(self):
        ...
        error1 = {"field-errors": {"email": u"Invalid email address: 💩"}}
        # https://github.com/MongoEngine/mongoengine/pull/1384
        error2 = {"field-errors": {"email": u"Invalid Mail-address: 💩"}}
        self.assertTrue(errors == error1 or errors == error2)
    ...
    def test_restricted_auth(self):
        ...
        resp = self.app.delete(
            "/restricted/{}/".format(data["id"]), data=json.dumps(post)
        )
        # Should work
        response_success(resp, code=200)
    ...
    def test_dbref_vs_objectid(self):
        ...
        resp = self.app.get("/b/{}/".format(dbref_obj["id"]))
        response_success(resp)
        dbref_obj = resp_json(resp)

        resp = self.app.get("/c/{}/".format(objectid_obj["id"]))
        response_success(resp)
        objectid_obj = resp_json(resp)
        ...
        resp = self.app.put(
            "/b/{}/".format(dbref_obj["id"]), data=json.dumps({"ref": a2["id"]})
        )
        response_success(resp)

        resp = self.app.put(
            "/c/{}/".format(objectid_obj["id"]), data=json.dumps({"ref": a2["id"]})
        )
        response_success(resp)
